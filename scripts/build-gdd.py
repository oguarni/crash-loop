#!/usr/bin/env python3
"""Build the crash-loop Game Design Document PDF from its Markdown source.

Usage:
    python3 scripts/build-gdd.py [path/to/GDD.md]

Defaults to GDD_crash-loop_Three-Way-Merge.md in the repo root and writes a PDF
with the same basename next to it. Requires xelatex and the Inter + Fira Code
fonts. The Markdown is the single source of truth; rerun this after editing it.
"""
import re, sys, pathlib, tempfile, subprocess, shutil

ROOT = pathlib.Path(__file__).resolve().parent.parent
MD = pathlib.Path(sys.argv[1]) if len(sys.argv) > 1 else ROOT / "GDD_crash-loop_Three-Way-Merge.md"
...
\hypersetup{unicode=true,
  pdftitle={crash-loop --- Game Design Document},
...
\titlecontents{section}[1.6em]{\addvspace{2pt}\headfont}
  {\contentslabel{1.6em}}{}{\titlerule*[0.6pc]{.}\contentspage}
\titlecontents{subsection}[3.6em]{\small}
...
\fancypagestyle{main}{%
  ...
  \fancyhead[R]{\footnotesize\color{charcoal} Game Design Document}
...
\textcolor{phosphor}{operator@helix}:\textcolor{amber}{\char`~}\$ ./boot --incident\\[4pt]
\textcolor{greenink!60!bone}{[ok]}~ ingress online\\
\textcolor{greenink!60!bone}{[ok]}~ sim engine: deterministic\\
\textcolor{alert}{[!!]}~ svc-cart: CrashLoopBackOff\\[12pt]
{\headfont\bfseries\Huge\color{phosphor} crash-loop}\\[6pt]
{\color{bone} Puzzle 2D de Infraestrutura e Automa\c{c}\~ao}
\end{tcolorbox}
\vspace{1.3cm}

{\large\color{bone}\textbf{Game Design Document}}\\[7pt]
...
Vers\~ao 1.0 \quad\textbullet\quad 2026-06-27 \quad\textbullet\quad Prot\'otipo jog\'avel: L01--L02 (TypeScript + HTML5 Canvas)}
\vspace{0.6cm}
\end{titlepage}
...
build = pathlib.Path(tempfile.mkdtemp(prefix="gdd-build-"))
(build / "gdd.tex").write_text(PREAMBLE + "\n" + BODY + FOOTER, encoding="utf-8")
for _ in range(2):  # twice so the table of contents resolves
    proc = subprocess.run([xelatex, "-interaction=nonstopmode", "-halt-on-error", "gdd.tex"],
                          cwd=build, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
pdf = build / "gdd.pdf"
if proc.returncode != 0 or not pdf.exists():
    sys.exit(f"error: xelatex failed; inspect {build/'gdd.log'}")
